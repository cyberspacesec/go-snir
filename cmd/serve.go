...
var serveCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// 确保报告目录存在
		reportPath, err := islazy.CreateDir(opts.Report.OutputPath)
		if err != nil {
		...
		http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
			serveIndex(w, r, screenshotPath, reportPath)
		})
		...
		// 启动服务器
		addr := fmt.Sprintf("%s:%d", opts.Report.Host, opts.Report.Port)
...
// serveIndex 处理首页请求
func serveIndex(w http.ResponseWriter, r *http.Request, screenshotPath, reportPath string) {
	// 如果请求的不是根路径，返回404
	if r.URL.Path != "/" {
		http.NotFound(w, r)
		return
